...
sAll<-read.table("Real_Mix_learning.csv", sep="",header=T) 
...
levels(sAll$cond)
c1<-"A 200µg/L|P 1mg/L"
c2<-"Control N°6"
c3<-"A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L"

levels(sAll$cond)
levels(sAll$cond) <- c(c1,c2,c3)

sAll$cond<-factor(sAll$cond,levels=c("Control N°6",
                "A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L",
                "A 200µg/L|P 1mg/L"))
levels(sAll$cond)
```
...
st3<-sf3 %>% filter(cond=="A 200µg/L|P 1mg/L")
st1<-sf3 %>% filter(cond=="Control N°6")
st2<-sf3 %>% filter(cond=="A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L")

ggplot(NULL) + 
  geom_smooth(data=st1, aes(y=dY, x=cat, group=1, color="Control N°6"), method="gam",
              formula = y ~ s(x, bs="ps", fx=FALSE, k=-1)) +
  geom_smooth(data=st2, aes(y=dY, x=cat, group=1, color="A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L"), method="gam",
              formula = y ~ s(x, bs="ps", fx=FALSE, k=-1)) +
  geom_smooth(data=st3, aes(y=dY, x=cat, group=1, color="A 200µg/L|P 1mg/L"), method="gam",
              formula = y ~ s(x, bs="ps", fx=FALSE, k=-1)) +
  geom_point(data=st1, aes(y=dY, x=cat, color="Control N°6"), size=3, shape=15) +
  geom_point(data=st2, aes(y=dY, x=cat, color="A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L"), size=3, shape=15) +
  geom_point(data=st3, aes(y=dY, x=cat, color="A 200µg/L|P 1mg/L"), size=3, shape=15) +
  theme_classic() +
  ...
  scale_color_manual(values = c("Control N°6"="#54C6CC", "A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L"="#008F00", 
                                "A 200µg/L|P 1mg/L"="#A5C38C"))


```

## 6) Plot trials 1 and Test --> Figure 5 D)
...
levels(sx$cond)
sx$cond<-factor(sx$cond,levels=c("Control N°6",
                "A 200µg/L|P 1mg/L",
                "A 0.126\u00b5g/L|G 2.3\u00b5g/L|P 6\u00b5g/L"
                ))
levels(sAll$cond)
...
ggplot(sx, aes(x=cat,y=dY))+
  ...
  scale_color_manual(values=c("#54C6CC","#008F00","#A5C38C"))+
